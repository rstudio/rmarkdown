...
Note that parameterized reports are a new feature of R Markdown and currently require the latest version of the **knitr** package and the **development version** of the R Markdown package. To ensure you have the required versions:
...
A version of R Markdown that supports parameterized reports will be released to CRAN soon.
...
Note that the `start` parameter includes a special `!date` annotation. This is explained further in the [Parameter Types] section below.

The declared parameters are automatically made available within the knit environment as components of a list named `params`. For example, the values of the above two parameters can be accessed with the following R code:
...
All of the standard R types that can be parsed by the `yaml::yaml.load` function are supported. These types are used implicitly based on the value provided so no special type designater is required. Built-in types include `character`, `integer`, `numeric`, and `logical`.
