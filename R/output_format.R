...
#'   if this is `TRUE` then `clean_supporting` will always be
#'   `FALSE`.
...
#'   receives the `input` (input filename passed to `render`) and
#'   `...` (for future expansion) arguments.
#' @param post_knit An optional function that runs after knitting which
#'   receives the `metadata`, `input_file`, `runtime`, and `...`
...
#'   original `input_file`, its `encoding`, and the intermediates
...
#'   and `verbose` parameters, and can return an alternative
#'   `output_file`.
#' @param on_exit A function to call when `rmarkdown::render()` finishes
...
#'
#' The `from` argument should be used very cautiously as it's
...
#' @param ext File extension (e.g. ".tex") for output file (if `NULL`
#'   chooses default based on `to`). This is typically used to force
#'   the final output of a latex or beamer conversion to be `.tex`
#'   rather than `.pdf`.
#' @return An list that can be passed as the `pandoc` argument of the
#'   [output_format()] function.
#' @seealso [output_format], [rmarkdown_format]
#' @export
...
#' that can be passed as the `from` argument of [pandoc_options].
#'
...
#'
#' \tabular{l}{
#' `+autolink_bare_uris` \cr
#' `+ascii_identifier` \cr
#' `+tex_math_single_backslash` \cr
#' }
#'
...
#' }
#' @seealso [output_format], [pandoc_options]
#' @export
...
#' @return A named list with a `name` value containing the format
#'   name and an `options` value that is a list containing all the options
...
#' @param input Input file (Rmd or plain markdown)
#' @param output_format Name of output format (or `NULL` to use
