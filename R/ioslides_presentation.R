...
#'   which currently works for all widely used retina displays).
#'   Set to \code{NULL} to prevent retina scaling. Note that this will always
#'   be \code{NULL} when \code{keep_md} is specified (this is because
#'   \code{fig_retina} relies on outputting HTML directly into the markdown
#'   document).
#' @param fig_caption \code{TRUE} to render figures with captions.
#' @param dev Default graphics device to use for figure output.
...
#' @param template Path to a pandoc template to use instead of the default
#'   bundled template.
...
#'   If a \code{before_body} include is specified then it will replace the
#'   standard title slide entirely.
