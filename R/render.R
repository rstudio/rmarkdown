...
render <- function(input,
  ...
  files_dir <-file.path(byproducts_dir %||% output_dir,
    knitr_files_dir(basename(output_file)))
  ...
  if (tolower(tools::file_ext(input)) %in% c("r", "rmd", "rmarkdown")) {
    ...
    if (!is.null(byproducts_dir)) {
      cache_dir <- file.path(byproducts_dir, cache_dir)
    }
