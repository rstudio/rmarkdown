...
#' }
#' The first call to \code{\link{render}} creates an HTML document, whereas the
#' second creates a PDF document.
#'
...
#' Note that the R comment used above to add a title, author, and date includes
#' a single-quote as a special prefix character. This is a \pkg{roxygen2} style
#' comment, and it's actually possible to include many such comments in an R
#' script, all of which will be converted to markdown content within the
...
#' script to an Rmd file. The \code{spin} function also enables you to add knitr
