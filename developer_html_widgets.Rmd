...
The **htmltools** package includes helper functions that make it straightforward to implement both of these print methods and share the underlying code used to generate HTML and specify dependencies.
...
HTML dependencies are defined using the `htmltools::html_dependency` function and include a name, version, path and optional meta, script, and stylesheet elements to include within the document head. Multiple instances of a dependency with the same name (e.g. "jquery") are merged into a single dependency and the most recent version specified is used.
