...
You can also use CSS to modify or even replace entirely the built-in visual themes. See the [Custom CSS] section below for details.

## Figures

There are a number of options that affect the output of figures within HTML documents:

* `fig_width` and  `fig_height` can be used to control the default figure width and height (7x5is used if not values are specified)
