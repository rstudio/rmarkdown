...
Support for floating table of contents is only available within the development version of the **rmarkdown** package. You can install the development version as follows:
...
Support for tabsets is only available within the development version of the **rmarkdown** package. You can install the development version as follows:
...
Support for code folding is only available within the development version of the **rmarkdown** package. You can install the development version as follows:
...
1. Create a navigation bar (**\_navbar.html**) to provide a way to navigate between the pages on the website. Here are two examples of navigation bars:

    - Simple: <https://github.com/rstudio/rmarkdown-website/blob/master/_navbar.html>
    - More Complex: <https://github.com/rstudio/rmarkdown/blob/gh-pages/_navbar.html>
    
    <br/>
    Full documentation on syntax of Bootstrap navigation bars can be found here: <http://getbootstrap.com/components/#navbar>.

2. Create a shared options file (**\_output.yaml**) to ensure common options across all pages within the site. This file will typically specify that documents are not `self_contained` and define a common `lib_dir` for JavaScript and CSS dependencies. It may also include common page elements (e.g. Google Analytics or Disqus comments).
...
3. Add documents to the website (being sure to add them to the **\_navbar.html**). Since common output options are specified in **\_output.yaml** the front matter inside individual documents then only needs to specify the document title:
...
4. Add a Makefile which can be used to build all the pages within the site (here is an [example Makefile](https://github.com/rstudio/rmarkdown-website/blob/master/Makefile) which does this.)

Here is a simple example of an R Markdown website that includes all of the components described above: <https://github.com/rstudio/rmarkdown-website>. 

#### Development Version

Support for creating websites as described above is only available within the development version of the **rmarkdown** package. You can install the development version as follows:
