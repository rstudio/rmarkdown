...
It is of course very convenient to be able to share values between execution contexts, and below we'll describe a couple of hybrid contexts that can be used accomplish this.
...
For the above example the cache will be invalidated if either the code in the chunk changes or the modification time of the "data.csv" file changes (this is accomplished using the `cache.extra` option).

You can also invalidate an existing cache by removing the `_cache` directory associated with your R Markdown document. There are many additional options available for controlling caching behavior, see the [knitr caching](http://yihui.name/knitr/demo/cache/) documentation for details.
