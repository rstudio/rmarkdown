...
This section covers advanced topics including conventions around referencing external web files (e.g. images, CSS stylesheets, etc.), the creation of re-usable components for `runtime: shiny_prerendered` documents, and gaining finer grained control over the rendering step. Understanding these topics is not required for everyday usage so feel free to skip them now and return to them only if and when necessary.
