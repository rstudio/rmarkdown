...
To use `runtime: shiny_prerendered` you need to install the development version of the [rmarkdown](http://rmarkdown.rstudio.com) package. You can do this as follows:

```{r, eval=FALSE}
devtools::install_github("rstudio/rmarkdown")
```

If you are running RStudio you should also download the Daily Build of RStudio (v1.0.103 or later) as it includes features that allow you easily run and preview `runtime: shiny_prerendered` documents within the IDE. Download links for the RStudio Daily Build are here:

- [Windows 7+](https://dailies.rstudio.com/rstudio/windows/)
- [Mac OS X 10.6+](https://dailies.rstudio.com/rstudio/mac/)
- [Ubuntu 12.04+](https://dailies.rstudio.com/rstudio/ubuntu/amd64/) 
...
Assuming you are using the current RStudio Daily Build ([Windows](https://dailies.rstudio.com/rstudio/windows/), [Mac OS X](https://dailies.rstudio.com/rstudio/mac/),  ([Ubuntu](https://dailies.rstudio.com/rstudio/ubuntu/amd64/)) you can execute a `runtime: shiny_prerendered` document locally using the **Run Document** command on the editor toolbar, or use the keyboard shortcut Ctrl+Shift+K (Cmd+Shift+K on Mac):
...
This section covers advanced topics including gaining finer grained control over the rendering step and the creation of re-usable components for `runtime: shiny_prerendered` documents. Understanding these topics is not required for everyday usage so feel free to skip them now and return to them only if and when necessary.
...
You may have combinations of Shiny UI and server code that you use frequently and wish to either use multiple times within a document or share across multiple documents. For this scenario it is convenient to be able to create a single function call that injects both the Shiny UI and Shiny Server code required into the document. This section describes how to accomplish this.

#### Shiny Modules

The starting point for creating components is **Shiny Modules**, which are pieces of a Shiny application can be easily reused, whether across different apps, or multiple times within in a single app. This article won't cover the basics of Shiny modules. Please see the [Shiny Modules](http://shiny.rstudio.com/articles/modules.html) article and [example application](http://shiny.rstudio.com/gallery/module-example.html) to learn more.

Here's a very simple Shiny module that we'll demonstrate how to incorporate into a `runtime: shiny_prerendered` document:

**worldPhones.R**

<div id="shiny-worldphones-module"></div>
<script type="text/javascript">loadSnippet('shiny-worldphones-module', 'r')</script>

Here is a simple `runtime: shiny_prerendered` document that does nothing but include the `world_phones.R` module:

<div id="shiny-worldphones-module-call"></div>
<script type="text/javascript">loadSnippet('shiny-worldphones-module-call', 'r')</script>

Here's what the resulting document looks like:

![](images/shiny-prerendered-module.png){.screenshot width=638 height=574 style="margin-bottom: 20px;"}


#### Module Wrapper Functions

In the previous example we invoked the Shiny module with a pair of calls: one in a rendering context for the UI and one in a server context for the Shiny server logic. Ideally, we could enable our users to invoke the module with a single function call. 

This is possible using the `rmarkdown::shiny_prerendered_chunk` function. This function enables you to emit a server-based chunk as a side effect, enabling you to include both UI and server components of a module within a single function call. 

Here's a new `worldPhones` function that demonstrates:

<div id="shiny-worldphones-component"></div>
<script type="text/javascript">loadSnippet('shiny-worldphones-component', 'r')</script>

Now we can re-write our document to exclude the `context="server"` chunk entirely (since it's emitted automatically by the new `worldPhones` function):

<div id="shiny-worldphones-module-component-call"></div>
<script type="text/javascript">loadSnippet('shiny-worldphones-module-component-call', 'r')</script>

Note that the above examples demonstrate defining the Shiny module within an R script (worldPhones.R) that is sourced into the Rmd. It is naturally also possible (and even preferred for wider distribution) to define the module functions within an R package.
