...
The call to the `rmarkdown::shiny_module` function will result in both the module's shiny UI function being called (and as a result the UI being rendered within the document) as well as the required call to the module's server function automatically being included in a `context="server"` chunk within the rendered document.
...
Note that the above example demonstrates defining the Shiny module within an R script (worldPhones.R) that is sourced into the Rmd. It is naturally also possible (and even preferred for wider distribution) to define the module functions within an R package.
