...
The `output_format` argument of `render` determines which format a .Rmd file will be rendered to. For example, the code below will render [6-output.Rmd](demos/6-output.Rmd) to a Microsoft Word document: 

```{r eval = FALSE}
render("6-output.Rmd", output_format = "word_document")
```

If you do not select a format, R Markdown will render the file to its default format, which you can set in the `output` field of a .Rmd file's header. A file with this header would be rendered to an HTML document by default: 
...
Each output format is implemented as a function in R. You can customize the output by passing arguments to the function as sub-values of the `output` field. For example, the file below would render with a floating table of contents.
...
To learn which arguments a format takes, read its help page in R, e.g. `?html_document`.


***

## [Continue to Notebooks](lesson-9.html)
