...
A `\newpage` or `\pagebreak` command in a rmarkdown document with output as HTML will be converted by default in this html code with inline style using CSS rule [`page-break-after`](https://www.w3schools.com/cssref/pr_print_pageba.asp)
...
To get more flexibility, you can use a HTML class and some custom CSS instead of an inline style. You need to add a metadata filed `newpage_html_class` in your yaml header to set the class.
...
This customisation can also be achieved by setting the environnement variable `PANDOC_NEWPAGE_HTML_CLASS` in the R session that will render the document (or in `.Renviron` for example)
...
As the previous one, this example will lead to a two page document, with first header on the second page.
...
Before pandoc 2.0, [using filter](https://pandoc.org/filters.html) with pandoc was already available through programs. `pandoc-citeproc` is an example used to deal with citations.
